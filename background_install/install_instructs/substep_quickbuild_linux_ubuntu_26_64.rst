...
This is the quick form of AFNI setup for Ubuntu 26.04.  It includes
...
#. **Download the command scripts.** 
   ...
   .. code-block:: none
      ...
      cd
      curl -O https://raw.githubusercontent.com/afni/afni/master/src/other_builds/OS_notes.linux_ubuntu_26_64_a_admin.txt
      curl -O https://raw.githubusercontent.com/afni/afni/master/src/other_builds/OS_notes.linux_ubuntu_26_64_b_user.tcsh
      curl -O https://raw.githubusercontent.com/afni/afni/master/src/other_builds/OS_notes.linux_ubuntu_26_64_c_nice.tcsh
...
#. **Run command scripts.**
   ...
   *For bash users:* 
   ...
   A. Copy+paste the following (**requires** ``sudo`` with password)::
        ...
        sudo bash OS_notes.linux_ubuntu_26_64_a_admin.txt 2>&1 | tee o.ubuntu_26_a.txt

   #. Copy+paste the following (**no** ``sudo``)::

        tcsh OS_notes.linux_ubuntu_26_64_b_user.tcsh 2>&1 | tee o.ubuntu_26_b.txt

   #. (optional, but useful) To niceify your terminal, copy+paste the
      following (**no** ``sudo``)::

        tcsh OS_notes.linux_ubuntu_26_64_c_nice.tcsh 2>&1 | tee o.ubuntu_26_c.txt

   *For tcsh or csh users:* 

   A. Copy+paste the following (**requires** ``sudo`` with password)::

        sudo bash OS_notes.linux_ubuntu_26_64_a_admin.txt |& tee o.ubuntu_26_a.txt

   #. Copy+paste the following (**no** ``sudo``)::

        tcsh OS_notes.linux_ubuntu_26_64_b_user.tcsh |& tee o.ubuntu_26_b.txt
