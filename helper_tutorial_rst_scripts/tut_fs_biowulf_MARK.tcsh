...
#:TITLE: Run FS *on a cluster* (NIH's Biowulf) with AFNI
...
#:SECTION: Module loading FS (and other things) in a script
...
#:SECTION: Using scratch disks as temporary directories
...
#:SECTION: A general biowulf script
...
#:IMAGE:  Example Biowulf Dashboard job info
          for_fs_biowulf/fs_biowulf_usage.png
#:IMCAPTION: Biowulf usage for memory (top) and CPUs (bottom) about 2
             hours into a ``recon-all`` run.  I had allocated 4 CPUs
             (useful!) and 10 GB (wasteful!).

TEXTBLOCK

#:SECTION: Looping over a group of subjects
